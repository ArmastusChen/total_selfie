...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.proj_out=nn.Linear(1024 * 4 , 768)
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                # flatten it
                input_num = c.shape[1] // 3
                bz = c.shape[0]
                c = c.view(bz * input_num, 3, c.shape[2], c.shape[3])
                # print(c.shape)
                
                c = self.cond_stage_model.encode(c)
                # print(c.shape)

                c = c.view(bz, input_num, c.shape[1], c.shape[2])
                # print(c.shape)

                c = c.view(bz, 1, input_num*c.shape[-1])
                # print(c.shape)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # print(z.shape, z_inpaint.shape, z_new.shape)
    ...
    def log_images(self, batch, N=4, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc
            if ismap(xc):
                # input_num = xc.shape[1] // 3 
                # cond_list = []
                # for kk in range(input_num):
                #     cond_list.append(self.to_rgb(xc[:,kk*3:(kk+1)*3,:,:]))

                # cond_list = torch.cat(cond_list,dim=1)
                log["original_conditioning"] = self.to_rgb(xc)
                # print(log["original_conditioning"].shape)
                # exit()
