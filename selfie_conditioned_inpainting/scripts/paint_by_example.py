...
class PaintByExampleImageEncoder(CLIPPreTrainedModel):
    def __init__(self, config, input_num=4,proj_size=None):
        ...
        self.proj_out = nn.Linear(config.hidden_size * input_num, self.proj_size)
    ...
    def forward(self, pixel_values, return_uncond_vector=False):
        ...
        bz = latent_states.shape[0] // self.input_num
        # print(latent_states.shape )
        # exit()
        latent_states = latent_states.reshape(bz, self.input_num, -1)
        latent_states = latent_states.reshape(bz, 1, -1)
