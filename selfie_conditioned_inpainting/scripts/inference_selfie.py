    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                ref_tensors = []
                for part in ['face', 'top', 'bottom', 'shoes']:
                    try:
                        ref_p = Image.open(f'{opt.reference_path}_{part}.png').convert("RGB").resize((224,224))
                    except:
                        ref_p = Image.open(f'{opt.reference_path}_{part}.jpg').convert("RGB").resize((224,224))
                    ref_tensor=get_tensor_clip()(ref_p)
                    ref_tensor = ref_tensor.unsqueeze(0)
                    ref_tensors.append(ref_tensor)

                ref_tensors = torch.cat(ref_tensors, dim=1)
                ...
                for seed in range(100):
                    ...
                    c = model.get_learned_conditioning(ref_tensors.to(torch.float16))
                    c = model.proj_out(c)
                    ...
                    if not opt.skip_save:
                        for i,x_sample in enumerate(x_checked_image_torch):
                            ...
                            img.save(os.path.join(grid_path, 'grid-'+filename[:-4]+'_'+str(seed)+'.jpg'))
                            ...
                            img = put_watermark(img, wm_encoder)
                            img.save(os.path.join(result_path, filename[:-4]+'_'+str(seed)+".jpg"))
                            ...
                            GT_img.save(os.path.join(sample_path, filename[:-4]+'_'+str(opt.seed)+"_GT.jpg"))
                            inpaint_img=255.*rearrange(all_img[1], 'c h w -> h w c').numpy()
                            inpaint_img = Image.fromarray(inpaint_img.astype(np.uint8))
                            inpaint_img.save(os.path.join(sample_path, filename[:-4]+'_'+str(opt.seed)+"_inpaint.jpg"))
                            ref_img=255.*rearrange(all_img[2], 'c h w -> h w c').numpy()
                            ref_img = Image.fromarray(ref_img.astype(np.uint8))
                            ref_img.save(os.path.join(sample_path, filename[:-4]+'_'+str(opt.seed)+"_ref.jpg"))
