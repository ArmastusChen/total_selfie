...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            print(k)
            if k == 'original_conditioning':
                input_num = images[k].shape[1] // 3 
                images[k] = images[k].reshape(-1, input_num, 3, images[k].shape[2], images[k].shape[3])
                images[k] = images[k].transpose(0, 1).reshape(-1, 3, images[k].shape[3], images[k].shape[4])
            ...
            filename = "gs-{:06}_e-{:06}_b-{:06}_{}.jpg".format(
    ...
    def check_frequency(self, check_idx):
        if ((check_idx % self.batch_freq) == 0 or (check_idx in self.log_steps)) and (
            ...
            # print("Logging images at step {}".format(check_idx))
            # print(self.batch_freq)
