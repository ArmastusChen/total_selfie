...
def prepare_mask_and_masked_image(image, mask):
    ...
    if isinstance(image, torch.Tensor):
        ...
        # selfie_conditioned_inpainting inverses the mask
        mask = 1 - mask
...
class PaintByExamplePipeline(DiffusionPipeline):
    ...
    def __call__(
        self,
        example_images: Union[torch.FloatTensor, PIL.Image.Image],
        ...
        lora_scale: Optional[float] = None,
        clip_skip: Optional[int] = None,
        blending_step: int = 30,
        mask_image_dilate: Optional[torch.FloatTensor] = None,
        **kwargs,
        ...
        black_mask = Image.new('RGB', (512, 512), (0, 0, 0))

        _, unmasked_image = prepare_mask_and_masked_image(image, black_mask)

        if mask_image_dilate is not None:
            mask_image_dilate, _ = prepare_mask_and_masked_image(image, mask_image_dilate)
        ...
        # 3. Check inputs
        self.check_inputs(example_images[0], height, width, callback_steps)

        # 4. Encode input image
        image_embeddings = self._encode_image(
            example_images, device, num_images_per_prompt, do_classifier_free_guidance
        )
        ...
        _, unmasked_image_latents = self.prepare_mask_latents(
            mask,
            unmasked_image,
            batch_size * num_images_per_prompt,
            height,
        ...
        if mask_image_dilate is not None:
            mask_image_dilate, _ = self.prepare_mask_latents(
                mask_image_dilate,
                unmasked_image,
                batch_size * num_images_per_prompt,
                height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input_control = latent_model_input
                latent_model_input = torch.cat([latent_model_input, masked_image_latents, mask], dim=1)

            
                control_model_input = latent_model_input_control
                ...
                if i < blending_step:
                    init_latents_proper = self.scheduler.add_noise(unmasked_image_latents[:1], noise, torch.tensor([t]))
                    if mask_image_dilate is not None:
                        cur_mask = mask_image_dilate[:1] 
                    else:
                        cur_mask = mask[:1] 

                    alpha = 1
                    latents = (init_latents_proper *  cur_mask * alpha +  latents *  cur_mask * (1-alpha)) + (latents * (1 - cur_mask))
